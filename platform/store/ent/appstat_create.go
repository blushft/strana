...
func (asc *AppStatCreate) Save(ctx context.Context) (*AppStat, error) {
	...
	if _, ok := asc.mutation.Sessions(); !ok {
		return nil, &ValidationError{Name: "sessions", err: errors.New("ent: missing required field \"sessions\"")}
	}
	if _, ok := asc.mutation.BouceRate(); !ok {
		return nil, &ValidationError{Name: "bouce_rate", err: errors.New("ent: missing required field \"bouce_rate\"")}
	}
	if _, ok := asc.mutation.KnownDurations(); !ok {
		return nil, &ValidationError{Name: "known_durations", err: errors.New("ent: missing required field \"known_durations\"")}
	}
