...
func (nq *NetworkQuery) QueryEvents() *EventQuery {
	query := &EventQuery{config: nq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(network.Table, network.FieldID, nq.sqlQuery()),
...
func (nq *NetworkQuery) sqlAll(ctx context.Context) ([]*Network, error) {
	...
	if query := nq.withEvents; query != nil {
		...
		query.Where(predicate.Event(func(s *sql.Selector) {
			s.Where(sql.InValues(network.EventsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.event_network
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_network" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_network" returned %v for node %v`, *fk, n.ID)
			}
