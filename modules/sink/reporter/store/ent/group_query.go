...
func (gq *GroupQuery) QueryEvents() *EventQuery {
	query := &EventQuery{config: gq.config}
...
func (gq *GroupQuery) sqlAll(ctx context.Context) ([]*Group, error) {
	...
	if query := gq.withEvents; query != nil {
		...
		query.Where(predicate.Event(func(s *sql.Selector) {
			s.Where(sql.InValues(group.EventsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.event_group
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "event_group" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "event_group" returned %v for node %v`, *fk, n.ID)
			}
